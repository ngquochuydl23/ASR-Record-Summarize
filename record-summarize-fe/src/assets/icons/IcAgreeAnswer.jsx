import { colors } from "@/theme/theme.global";
import { SvgIcon } from "@mui/material";

const IcAgreeAnswer = () => (
  <SvgIcon sx={{ height: '15px', width: '15px', ml: '5px' }}>
    <svg version="1.1" id="Layer_1" 
      viewBox="0 0 512 512" >
      <polygon fill="#BFDCFF" points="0,504.205 0,192.549 113.523,192.549 135.784,348.378 113.523,504.205 " />
      <path fill="#FFDA03" d="M512,231.509c0-21.516-17.441-38.957-38.957-38.957h-58.999l-22.261,77.913v155.826l22.261,77.913
	h11.13c21.515,0,38.957-17.441,38.957-38.957c0-21.516-17.441-38.957-38.957-38.957h15.955c21.515,0,38.957-17.441,38.957-38.957
	c0-21.516-17.441-38.957-38.957-38.957h15.957c21.515,0,38.957-17.441,38.957-38.957c0-21.516-17.441-38.957-38.957-38.957h15.957
	C494.559,270.466,512,253.024,512,231.509z"/>
      <path fill="#FFDA03" d="M414.044,504.205h-256l-22.261-155.827l22.261-155.827c80.52,0,142.955-65.274,142.955-145.793
	c0-21.52,17.445-38.964,38.964-38.964s38.964,17.445,38.964,38.964c0,53.21-13.067,104.182-39.45,144.683v1.113h74.566
	L414.044,504.205L414.044,504.205z"/>
      <rect x="113.519" y="192.555" fill="#BFDCFF" width="44.522" height="311.652" />
    </svg>
  </SvgIcon>
)

export default IcAgreeAnswer;